...
import { setToggle, setToggleNavbar } from "../../redux/actions";
import logo from "../../assets/corierHub_logo.png"

function Navbar() {
  const dispatch = useDispatch();
  const { toggle, toggleNavbar } = useSelector((state) => state);
  ...
  return (
    <div className=' z-50 fixed left-0 top-0 right-0 h-[76px] px-6 py-4 backdrop-blur-xl bg-white/30 border-b justify-between items-center gap-[20px] inline-flex'>
            <div className=" flex gap-6 items-center">
                {/* <img src={require('../admin-page/Screenshot_6.png')} alt="" /> */}
                <div className=" border-r pr-6 border-[#e4e4e4]">
                    <img src={logo} alt="" className="h-12 w-48"/>
                </div>
                <div className=" text-xl font-semibold leading-8 text-[#19191C]" onClick={handleToggle}>Dashboard</div>
            </div>
